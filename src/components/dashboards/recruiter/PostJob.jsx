...
const PostJob = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-background-dark text-white font-sans selection:bg-primary selection:text-white overflow-hidden relative">
      {/* Background Effects */}
      <div className="fixed inset-0 bg-hero-glow pointer-events-none z-0"></div>
      <div className="absolute top-20 right-[-50px] sm:right-0 w-[200px] h-[200px] sm:w-[400px] sm:h-[400px] bg-blue-600/10 rounded-full blur-[80px] pointer-events-none z-0"></div>
      <div className="absolute bottom-0 left-[-50px] sm:left-0 w-[200px] h-[200px] sm:w-[400px] sm:h-[400px] bg-purple-600/10 rounded-full blur-[80px] pointer-events-none z-0"></div>

      {/* --- Header --- */}
      <header className="flex-shrink-0 z-30 bg-[#1a1d23]/95 backdrop-blur-md border-b border-white/5 px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between relative">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 w-full">
        <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 animate-fade-in pb-8 sm:pb-12">
            ...
            <div className="space-y-6">
              ...
              <button
                onClick={() => {
                  const newJob = {
                    id: Date.now(),
                    role: "Senior Product Designer", // Using a placeholder for now as inputs aren't controlled fully
                    location: "Remote",
                    type: "Full Time",
                    postedDate: "Just now",
                    applicants: 0,
                  ...
                  navigate('/dashboard/recruiter/jobs', { state: { newJob } });
                }}
                className="w-full bg-primary hover:bg-primary-glow text-white font-bold py-4 rounded-xl shadow-[0_0_20px_rgba(0,230,191,0.4)] hover:shadow-[0_0_30px_rgba(0,230,191,0.6)] transition-all flex items-center justify-center gap-2">
                Post Job Now
